//
// Created by Rorschach on 2024/12/17.
//
...
Java_com_xinayida_ndksample_MainActivity_callbackFromJNI(JNIEnv *env,
                                                         jobject thiz) {
#if defined(__arm__)
...
//    returnenv->NewStringUTF(env, "Hello from JNI !  Compiled with ABI " ABI ".");
}
...
void queryRuntimeInfo(JNIEnv *env, jobject instance) {
    ...
    jmethodID versionFunc = env->GetStaticMethodID(
            g_ctx.jniHelperClz, "getBuildVersion", "()Ljava/lang/String;");
    if (!versionFunc) {
        LOGE("Failed to retrieve getBuildVersion() methodID @ line %d", __LINE__);
        return;
    }
    jstring buildVersion =
            (jstring) env->CallStaticObjectMethod(g_ctx.jniHelperClz, versionFunc);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jmethodID jniHelperCtor =
            env->GetMethodID(g_ctx.jniHelperClz, "<init>", "()V");
...
void *UpdateTicks(void *context) {
    ...
    jmethodID statusId = env->GetMethodID(
            pctx->jniHelperClz, "updateStatus", "(Ljava/lang/String;)V");
...
Java_com_xinayida_ndksample_MainActivity_startTicks(JNIEnv *env,
                                                    jobject instance) {
 ...
 */
extern "C" JNIEXPORT void JNICALL Java_com_xinayida_ndksample_MainActivity_stopTicks(
